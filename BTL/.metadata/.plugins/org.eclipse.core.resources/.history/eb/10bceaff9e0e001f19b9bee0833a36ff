...
public class PanelXe extends JPanel {
	...
	public PanelXe(XeFacade xeFacade) {
		...
		JLabel lblTenXeMay = new JLabel("Tên đĩa:");
		...
		JLabel lblMauXe = new JLabel("Loại đĩa:");
		...
		JLabel lblHangXe = new JLabel("Hãng đĩa:");
		...
		JLabel lblLoaiXe = new JLabel("Mô tả đĩa:");
		...
		JLabel lblNuocSX = new JLabel("Thể loại đĩa:");
		...
		JLabel lblSoKhung = new JLabel("Chất liệu:");
		...
		JLabel lblSoPK = new JLabel("Số lô:");
		...
		String[] headers = { "Mã đĩa", "Tên đĩa", "Hãng đĩa", "Mô tả đĩa", "Loại đĩa", "Thể loại đĩa", "Chất liệu", "Dung lượng",
				"Số lô", "Giá", "Số lượng tồn" };
		...
		btnThem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (valid()) {
					try {
						Xe xe = new Xe(txtMaXe.getText().trim(), txtNuocSX.getText().trim(), txtLoaiXe.getText(),
								Integer.parseInt(txtSoPK.getText().trim()), txtSoKhung.getText().trim(),
		...
		btnXoa.setBounds(211, 3, 227, 33);
		btnXoa.setIcon(new ImageIcon(img_Delete));
		pnMenuChucNang.add(btnXoa);

		JButton btnCapNhat = new JButton("Cập nhật đĩa");
		...
		btnCapNhat.setBounds(438, 3, 238, 33);
		...
		btnTaoMoi.setBounds(675, 3, 190, 33);
		btnTaoMoi.setIcon(new ImageIcon(img_New));
		pnMenuChucNang.add(btnTaoMoi);

		JLabel lblDSXe = new JLabel("DANH SÁCH ĐĨA");
		...
		btnTimKiem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					xoaHetTable();
					if (xeTK != null) {
						String[] rowData = { xeTK.getMaXe(), xeTK.getTenXe(), xeTK.getHangXe(), xeTK.getLoaiXe(),
								xeTK.getMauXe(), xeTK.getNuocSX(), xeTK.getSoKhung(), xeTK.getSoSuon(),
								xeTK.getSoPK() + "", dcf.format(xeTK.getDonGia()), xeTK.getSoLuongTon() + "" };
						...
						if (list.isEmpty()) {
							...
							for (Xe s : list) {
								String[] rowData = { s.getMaXe(), s.getTenXe(), s.getHangXe(), s.getLoaiXe(),
										s.getMauXe(), s.getNuocSX(), s.getSoKhung(), s.getSoSuon(), s.getSoPK() + "",
										dcf.format(s.getDonGia()), s.getSoLuongTon() + "" };
								modelXe.addRow(rowData);
							}
						}
					}
		...
		txtTimKiem.setText("Tìm theo mã đĩa, tên đĩa hoặc hãng");
		txtTimKiem.addFocusListener(new FocusAdapter() {
			@Override
			public void focusGained(FocusEvent e) {
				if (txtTimKiem.getText().equals("Tìm theo mã đĩa, tên đĩa hoặc hãng")) {
					txtTimKiem.setText("");
				} else {
			...
			public void focusLost(FocusEvent e) {
				if (txtTimKiem.getText().equals("")) {
					txtTimKiem.setText("Mã đĩa, tên đĩa, hãng");
				}
			}
		});
		...
		JLabel lblSoSuon = new JLabel("Dung lượng:");
	...
	public void updateTableData() {
		...
		try {
			...
			for (Xe s : list) {
				String[] rowData = { s.getMaXe(), s.getTenXe(), s.getHangXe(), s.getLoaiXe(), s.getMauXe(),
						s.getNuocSX(), s.getSoKhung(), s.getSoSuon(), s.getSoPK() + "", dcf.format(s.getDonGia()),
						s.getSoLuongTon() + "" };
				modelXe.addRow(rowData);
			}
			tableXe.setModel(modelXe);
