...
public class PanelDanhSachHopDong extends JPanel {
	 ...
	 * Create the panel.
	 */
	public PanelDanhSachHopDong(HopDongFacade hopDongFacade, XeFacade xeFacade, KhachHangFacade khachHangFacade,
			String maNVDangNhap) {
				...
				try {
					...
					if (hd != null) {
						...
						String[] rowData = { hd.getSoHopDong(), formatter.format(hd.getNgayHopDong()),
								hd.getNhanVienLapHD().getMaNV(), hd.getKhDaiDien().getMaKH(), hd.getThoiGianBH() + "",
								dcf.format(hd.getTongThanhTien()), dcf.format(hd.getTienPhaiTra()),
								dcf.format(hd.getTienThua()), tinhTrang };
						...
						for (HopDong hdTK : dsHD) {
							...
							String[] rowData = { hdTK.getSoHopDong(), formatter.format(hdTK.getNgayHopDong()),
									...
									dcf.format(hdTK.getTienPhaiTra()), dcf.format(hdTK.getTienThua()), tinhTrang };
							modelHD.addRow(rowData);
						}
					}
		...
		JButton btnXoa = new JButton("Xoá hoá đơn");
		...
		JButton btnCapNhat = new JButton("Cập nhật hoá đơn");
		...
		txtNgayLapHoaDon.setBounds(139, 72, 210, 24);
		...
		JLabel lblDanhSachChiTietHoaDon = new JLabel("CHI TIẾT HOÁ ĐƠN");
		...
		String[] headers1 = { "Mã đĩa", "Tên đĩa", "Hãng đĩa", "Số lượng", "Đơn giá", "Thành tiền" };
		...
		JButton btnXoaSP = new JButton("Xoá đĩa");
		...
		JButton btnThemXe = new JButton("Thêm đĩa");
		...
		JLabel lblTenXe = new JLabel("Tên đĩa:");
		...
		if (list.isEmpty()) {
			...
			try {
				for (ChiTietHopDong ct : list) {
					...
					String[] rows = { ct.getXe().getMaXe(), xe.getTenXe(), xe.getHangXe(), ct.getSoLuong() + "",
							dcf.format(ct.getDonGia()), dcf.format(ct.getThanhTien()) };
		...
		try {
			...
			for (HopDong hopDong : listHD) {
				...
				String[] row = { hopDong.getSoHopDong(), formatter.format(hopDong.getNgayHopDong()),
						...
						dcf.format(hopDong.getTienPhaiTra()), dcf.format(hopDong.getTienThua()), tinhTrang };
				modelHD.addRow(row);
			}
	...
	public void capNhatChiTietThanhToanTrongHopDong() {
		...
		for (ChiTietThanhToan chiTietThanhToan : dsCTTT) {
			hopDong.themChiTietThanhToan(chiTietThanhToan.getLanTra(), chiTietThanhToan.getTienTra(),
					chiTietThanhToan.getTenNguoiTra());
		}
	}
}
