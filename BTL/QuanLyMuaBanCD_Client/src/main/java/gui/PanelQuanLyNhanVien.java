...
//		cbLoaiNV.setEditable(false);
		...
		JLabel lblMaNhanVien = new JLabel("Mã NV:");
			...
			public void mouseClicked(MouseEvent e) {
				...
				try {
					...
					if (nvhc != null) {
						...
						cbLoaiNV.setText("Nhân viên hành chính");
...
//					}
		...
		pnlChucNang.setBounds(159, 249, 790, 38);
		add(pnlChucNang);
		...
		btnThem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (valid()) {
					try {
						if (cbLoaiNV.getText().equals("Nhân viên hành chánh")) {
							NhanVienHanhChanh nvhc = new NhanVienHanhChanh(txtMaNhanVien.getText().trim(),
									...
									txtPhongBan.getText().trim(), txtHocVan.getText().trim());
							TaiKhoan tk;
...
//									txtHoTen.getText().trim(), txtDiaChi.getText().trim(),
//									txtSoDienThoai.getText().trim(), txtChucVu.getText().trim(),
//									txtBacTho.getText().trim(), Integer.parseInt(txtKinhNghiem.getText().trim()));
...
//						}
		...
		btnXoa.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int hoiNhac = JOptionPane.showConfirmDialog((Component) e.getSource(), "Có chắn chắn xóa", "Chú ý",
...
//					}
					...
					try {
						if (cbLoaiNV.getText().equals("Nhân viên hành chánh")) {
							...
							if (nhanVienHanhChanhFacade.capNhatNhanVienHanhChanh(nvhc)) {
								lblMess.setText("Cập nhập thành công");
		...
		JButton btnTaoMoi = new JButton("Xóa trắng");
		...
		btnTimKiem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					if (nvhc != null) {
						String[] row = { nvhc.getMaNV(), nvhc.getTenNV(), nvhc.getDiaChiNV(), nvhc.getSoDienThoaiNV(),
								"Nhân viên hành chánh", nvhc.getChucVu(), "", "", nvhc.getPhongBan(),
								nvhc.getTrinhDoHocVan() };
						model.addRow(row);
					} else {
						List<NhanVienHanhChanh> listNVHC = nhanVienHanhChanhFacade
								.timNhanVienHanhChanhTheoTextSearch(keywords);
						for (NhanVienHanhChanh nvhcTK : listNVHC) {
							String[] row = { nvhcTK.getMaNV(), nvhcTK.getTenNV(), nvhcTK.getDiaChiNV(),
									nvhcTK.getSoDienThoaiNV(), "Nhân viên hành chánh", nvhcTK.getChucVu(), "", "",
						...
						if (nvkt != null) {
							String[] row = { nvkt.getMaNV(), nvkt.getTenNV(), nvkt.getDiaChiNV(),
									nvkt.getSoDienThoaiNV(), "Nhân viên kỹ thuật", nvkt.getChucVu(), nvkt.getBacTho(),
									nvkt.getSoNamKN() + "", "", "" };
							model.addRow(row);
						} else {
							List<NhanVienKyThuat> listNVKT = nhanVienKythuatFacade
									.timNhanVienKyThuatTheoTextSearch(keywords);
							for (NhanVienKyThuat nvktTK : listNVKT) {
								String[] row = { nvktTK.getMaNV(), nvktTK.getTenNV(), nvktTK.getDiaChiNV(),
										nvktTK.getSoDienThoaiNV(), "Nhân viên kỹ thuật", nvktTK.getChucVu(),
										nvktTK.getBacTho(), nvktTK.getSoNamKN() + "", "", "" };
								model.addRow(row);
							}
		...
		txtTimKiem.setText("Tìm theo mã nv, tên nv, sđt");
		txtTimKiem.addFocusListener(new FocusAdapter() {
			@Override
			public void focusGained(FocusEvent e) {
				if (txtTimKiem.getText().equals("Tìm theo mã nv, tên nv, sđt")) {
					txtTimKiem.setText("");
				} else {
		...
		lblBatTho.setBounds(588, 147, 103, 22);
		lblBatTho.setVisible(false);
		add(lblBatTho);
		...
		txtBacTho.setBounds(696, 147, 380, 24);
		...
		lblKinhNghiem.setBounds(588, 180, 103, 22);
		...
		txtKinhNghiem.setBounds(696, 180, 380, 24);
		...
		lblHocVan.setBounds(608, 116, 83, 22);
		add(lblHocVan);
...
//		cbLoaiNV.addItem("Nhân viên kỹ thuật");
		...
		add(cbLoaiNV);
		cbLoaiNV.setText("Nhân viên hành chánh");
	...
	public void updateTable() {
		...
		try {
			...
			for (NhanVienHanhChanh nvhc : dsNVHC) {
				String[] rowData = { nvhc.getMaNV(), nvhc.getTenNV(), nvhc.getDiaChiNV(), nvhc.getSoDienThoaiNV(),
						"Nhân viên hành chánh", nvhc.getChucVu(), "", "", nvhc.getPhongBan(), nvhc.getTrinhDoHocVan() };
				model.addRow(rowData);
			}

			List<NhanVienKyThuat> dsNVTHC = nhanVienKyThuatFacade.getDSNhanVienKyThuat();
			for (NhanVienKyThuat nvkt : dsNVTHC) {
				String[] rowData = { nvkt.getMaNV(), nvkt.getTenNV(), nvkt.getDiaChiNV(), nvkt.getSoDienThoaiNV(),
						"Nhân viên kỹ thuật", nvkt.getChucVu(), nvkt.getBacTho(), nvkt.getSoNamKN() + "", "", "" };
				model.addRow(rowData);
			}
