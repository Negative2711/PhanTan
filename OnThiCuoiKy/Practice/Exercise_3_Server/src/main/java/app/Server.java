	...
	public void run() {
		try {
			...
			while (true) {
				...
				switch (choice) {
					case 1:
						String title = in.readUTF();// Chờ title gởi qua
						System.out.println("Client title vua nhap la=" + title);
	
						List<Albums> listAlbumByGenre = albumService.listAlbumByGenre(title);
